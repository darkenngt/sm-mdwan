...
<div class="col-sm-3">{{ time.days }} <span>D</span></div>
<div class="col-sm-3">{{ time.hours }} <span>H</span></div>
<div class="col-sm-3">{{ time.minutes }} <span>M</span></div>
<div class="col-sm-3">{{ time.seconds }} <span>S</span></div>
</div>
`,
  styles: [``],
})
export class CountdownTimerSm implements OnInit {
  time!: {
    days: number;
    hours: number;
    minutes: number;
